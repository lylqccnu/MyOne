...
public class MaxPropRouter extends ActiveRouter {
	...
	/**成功传输消息后，应在（在接收主机上）调用此方法。
	 * 传输的消息将被放入消息缓冲区，除非该主机是消息的最终接收者。
	 * */
	@Override
	 ...
	 * 在传输完成之前调用方法ActiveRouter.update()。
	 */
	@Override
	...
	/**根据MaxProp的消息排序方案返回应删除的下一条消息（请参阅MaxPropTupleComparator）
	 ...
	 * 根据每次传输机会传输的平均字节数和缓冲区中消息的跳数计算并返回缓冲区拆分的当前阈值。
	 */
