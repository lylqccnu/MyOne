...
public abstract class ActiveRouter extends MessageRouter {
	...
	private EnergyModel energy;	//能量模式
	...
	/**当连接状态发生变化时调用。如果启用了能量模式，并且创建了到此节点的连接
	 * 那么能量衰减
	...
	/**检查路由是否"想"开始接收消息(即路由没有在传输，没有消息并且有空间接收消息)
	...
	/**返回消息缓冲区中最老的消息(按接收时间)(如果excludeMsgBeingSent为真则不发送该消息)
	...
	/**尝试按元组在列表中的顺序发送元组中提到的连接的消息，
	 * 直到其中一个连接开始传输或尝试了所有元组。
	...
	protected Tuple<Message, Connection> tryMessagesForConnected(
	 ...
	 /**遍历消息直到其他节点接收一个消息(或不接收任何消息)
	  * 如果一个传输开始，连接将包含在连接列表中
