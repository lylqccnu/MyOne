...
public abstract class MessageRouter {
	...
	/**如果此节点以最终收件人的身份接收了与给定消息具有相同ID的完整消息(至少一次)，则返回true
	...
	/** 如果消息被加入黑名单，则返回true，当运行在节点上的应用程序想要删除消息时，
	 * 消息将被加入黑名单。这确保了其他节点不会不断尝试向这个节点发送相同的消息，
	 * 而每次都被应用程序删除。
	...
	/**将消息放入传入消息缓存区。具有相同ID的两条消息根据节点进行区分
	...
	/**从传入消息缓存区中删除并返回具有指定ID的消息，如果没有找到该条消息则返回null
	...
	/**根据传入的消息ID判断当前传入的消息是否存在，存在则返回true否则返回false
